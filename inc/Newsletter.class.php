    ...
    public static function register($email) {
        if (!empty($_POST)) {
            ...
            if (self::$valid) {
                ...
                $existingSignup = $pdo->prepare("SELECT COUNT(*) FROM newsletter_people WHERE np_email='$email'");
                $existingSignup->execute();
                $data_exists = ($existingSignup->fetchColumn() > 0) ? true : false;

                if (!$data_exists) {
                    $sql = "INSERT INTO newsletter_people (np_email, np_date) VALUES (:email, :datetime)";
                    $q = $pdo->prepare($sql);
